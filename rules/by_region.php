...
class Exact16ToBeReplacedTeeth(SeparatedJawRuleCompleteInterface):
    ...
    function upper_jaw(self):
        to_be_replaced = self.schema.to_be_replaced(region=Region.upper_jaw())
        region = self.schema.get_teeth_in_region(Region.upper_jaw())
    ...
    function mandible(self):
        to_be_replaced = self.schema.to_be_replaced(region=Region.mandible())
        region = self.schema.get_teeth_in_region(Region.mandible())
        if len(to_be_replaced) == 16:
            self.identified_subsidies.append(
                rv_subsidy_4x(
...
class UnilateralFreeEndToBeReplacedTeethAtLeast1(
    ...
    Update: 01/02/2019. Now when a free end happens in both sides
    ...
    function upper_jaw(self):
        ...
        """
        unilateral_free_end = (
            self.schema.upper_jaw_left_free_end
            or self.schema.upper_jaw_right_free_end
        )
        ...
        if unilateral_free_end:
            self.identified_subsidies.append(
                rv_subsidy_3x(
                    {
                        "subsidy": "3.1",
                        "region": self.schema.get_teeth_in_region(
                            Region.upper_jaw()
                        ),
                        "applied_rule": self,
                    }
    ...
    function mandible(self):
        ...
        """
        unilateral_free_end = (
            self.schema.mandible_left_free_end
            or self.schema.mandible_right_free_end
        )
...
class UnilateralFreeEndToBeReplacedX7andX8(SeparatedJawRuleCompleteInterface):
    """
    In 28/01/2019 it was decided that this rule would also be applied
    ...
    function _unilateral_free_end_upper_jaw(self):
        """
        Unilateral free end only X7/X8 missing (CAVEAT from 28/01/2019).
        """
        right_end_tbr, left_end_tbr = False, False
        to_be_replaced_right = self.schema.to_be_replaced(region=Region(27, 28))
        to_be_replaced_left = self.schema.to_be_replaced(region=Region(18, 17))

        if len(to_be_replaced_right) == 2:
            # Just consider free end being X7/X8, if one more tooth
            # is also TBR it will not be considered
            if not self.schema.to_be_replaced(region=Region(26, 26)):
                right_end_tbr = True

        if len(to_be_replaced_left) == 2:
            if not self.schema.to_be_replaced(region=Region(16, 16)):
                left_end_tbr = True
    ...
    function _unilateral_free_end_mandible(self):
        ...
        """
        right_end_tbr, left_end_tbr = False, False
        to_be_replaced_right = self.schema.to_be_replaced(region=Region(38, 37))
        to_be_replaced_left = self.schema.to_be_replaced(region=Region(47, 48))

        if len(to_be_replaced_right) == 2:
            # Just consider free end being X7/X8, if one more tooth
            # is also TBR it will not be considered
            if not self.schema.to_be_replaced(region=Region(36, 36)):
                right_end_tbr = True

        if len(to_be_replaced_left) == 2:
            if not self.schema.to_be_replaced(region=Region(46, 46)):
                left_end_tbr = True
    ...
    function mandible(self):
        ...
        Check for unilateral free end in the mandible
        """
        unilateral_free_end = self._unilateral_free_end_mandible()
...
class Between5And12ToBeReplacedTeeth(SeparatedJawRuleCompleteInterface):
    ...
    function upper_jaw(self):
        ...
        to_be_replaced = self.schema.to_be_replaced(region=upper_jaw_without_x8)
    ...
    function mandible(self):
        ...
        to_be_replaced = self.schema.to_be_replaced(region=mandible_without_x8)
...
class Exact4ToBeReplacedTeethInterdentalGapWithAtLeastOneInPosteriorRegion(
    ...
    function upper_jaw(self):
        interdental_gap = self.schema.interdental_gap_with_to_be_replaced_count(
            Region.upper_jaw(), 4
        )
...
class Between2And3ToBeReplacedTeethInterdentalGapNeighboringAnotherGap(
    ...
    function execute_for(self, region):
        interdental_gaps = self.schema.all_interdental_gaps()

        for interdental_gap in interdental_gaps:
            if 2 <= interdental_gap.to_be_replaced_count <= 3:
                neighboring_gap = self.schema.neighboring_gap(interdental_gap)
                ...
                self.identified_subsidies.append(
                    rv_subsidy_3x(
                        {
                            "subsidy": "3.1",
                            "region": self.schema.get_teeth_in_region(region),
                            "applied_rule": self,
                        }
                    )
                )
...
class NeighboringInterdentalGapsCountRelativeToTheMainGapGreaterThan1(
    ...
    function execute_for(self, region):
        main_interdental_gap = self.schema.main_interdental_gap(region)
        all_gaps = self.schema.all_interdental_gaps(region=region)
        neighbors = []
        ...
        # Check Issue 310 for more details
...
class BilateralFreeEnd(SeparatedJawRuleCompleteInterface):
    ...
    function upper_jaw(self):
        if (
            self.schema.upper_jaw_left_free_end
            and self.schema.upper_jaw_right_free_end
        ):
            self.identified_subsidies.append(
                rv_subsidy_3x(
                    {
            ...
            tooth_32_left = self.schema.upper_jaw_right_free_end.teeth[0].left
            tooth_32_right = self.schema.upper_jaw_left_free_end.teeth[-1].right
    ...
    function mandible(self):
        if (
            self.schema.mandible_left_free_end
            and self.schema.mandible_right_free_end
        ):
            self.identified_subsidies.append(
                rv_subsidy_3x(
                    {
            ...
            tooth_32_left = self.schema.mandible_left_free_end.teeth[0].left
            tooth_32_right = self.schema.mandible_right_free_end.teeth[-1].right
...
class ExactToBeReplacedTeethInterdentalGapInFronRegion(
    ...
    function upper_jaw(self):
        ...
        # In Issue #373 (09/07/2019) we defined that we would change this
        # concept (that was previously defined #133) to:
        ...
        if (
            not Region(18, 16, self.schema).to_be_replaced_count == 3
            and not Region(26, 28, self.schema).to_be_replaced_count == 3
        ) {
            return False;
        }
        ...
        # minimum X7-X8
        if (
            Region(18, 16, self.schema).to_be_replaced_count == 3
            and not Region(27, 28, self.schema).to_be_replaced_count == 2
        ):
            return False
        ...
        # minimum X7-X8
        if (
            Region(26, 28, self.schema).to_be_replaced_count == 3
            and not Region(18, 17, self.schema).to_be_replaced_count == 2
        ):
            return False

        # As defined in Issue #133, missing tooth must be inside 12-22
        # area, that's the reason by us starting from 13-23 here.
        interdental_gap = self.schema.interdental_gap(
            region=Region(13, 23, self.schema)
        )
        ...
        if (
            len(
                self.schema.interdental_gaps(region=Region(13, 23, self.schema))
            )
            != 1
        ):
            return False
        ...
        tooth_12 = Region(12, 12, self.schema)
        tooth_22 = Region(22, 22, self.schema)
        ...
        teeth_32 = self.identified_subsidies.teeth(subsidy_code="3.2")
        if (
        ...
        if interdental_gap.to_be_replaced_count == self.to_be_replaced_count:
            self.identified_subsidies.append(
                {
...
class UnilateralFreeEndColateralInterdentalGap(
    ...
    function upper_jaw(self):
        ...
        # of the mouth.
        left_colateral_interdental_gap = self.schema.interdental_gap(
            region=Region(23, 28, self.schema)
        )
        ...
        if (
            ...
            if left_colateral_interdental_gap.to_be_replaced_count >= 2:
                if left_colateral_interdental_gap.to_be_replaced[0].number in [
                    ...
                    tooth_32_left = self.schema.upper_jaw_left_free_end.teeth[
                        -1
                    ].right
        ...
        # of the mouth.
        right_colateral_interdental_gap = self.schema.interdental_gap(
            region=Region(18, 13, self.schema)
        )

        if (
            self.schema.upper_jaw_right_free_end
            ...
            if right_colateral_interdental_gap.to_be_replaced_count >= 2:
                if right_colateral_interdental_gap.to_be_replaced[
                    ...
                    tooth_32_right = self.schema.upper_jaw_right_free_end.teeth[
                        0
                    ].left
    ...
    function mandible(self):
        ...
        # of the mouth.
        left_colateral_interdental_gap = self.schema.interdental_gap(
            region=Region(38, 33, self.schema)
        )
        ...
        if (
            ...
            if left_colateral_interdental_gap.to_be_replaced_count >= 2:
                if left_colateral_interdental_gap.to_be_replaced[-1].number in [
                    ...
                    tooth_32_left = self.schema.mandible_left_free_end.teeth[
                        0
                    ].left
        ...
        # of the mouth.
        right_colateral_interdental_gap = self.schema.interdental_gap(
            region=Region(43, 48, self.schema)
        )

        if (
            self.schema.mandible_right_free_end
            and right_colateral_interdental_gap
        ):
            if right_colateral_interdental_gap.to_be_replaced_count >= 2:
                if right_colateral_interdental_gap.to_be_replaced[0].number in [
                    ...
                    tooth_32_right = self.schema.mandible_right_free_end.teeth[
                        -1
                    ].right
...
class BilateralInterdentalGap(SeparatedJawRuleCompleteInterface):
    ...
    function upper_jaw(self):
        ...
        """
        if not self.identified_subsidies.exists(
            subsidy_code_startswith="3.1", region=Region.upper_jaw()
        ):
            return False
        # Check LEFT interdental gap
        left_interdental_gap = self.schema.interdental_gap(
            region=Region(18, 13, self.schema)
        )

        if left_interdental_gap is None:
            return False
        ...
        right_interdental_gap = self.schema.interdental_gap(
            region=Region(23, 28, self.schema)
        )

        if right_interdental_gap is None:
            return False
...
class BiggestInterdentalGapInFrontRegionExactToBeReplaced4(
    ...
    function _check_rule_in_region(self, region):
        biggest_interdental_gap = self.schema.biggest_interdental_gap()
    ...
    function upper_jaw(self):
        return self._check_rule_in_region(Region.upper_jaw_front())

    function mandible(self):
        return self._check_rule_in_region(Region.mandible_front())
...
class BiggestInterdentalGapExactToBeReplaced3(SeparatedJawRuleSimpleInterface):
    ...
    function execute_for(self, region):
        biggest_interdental_gap = self.schema.biggest_interdental_gap(region)
...
class BiggestInterdentalGapNeighboringGapExactToBeReplaced1(
    ...
    function execute_for(self, region):
        ...
        for gap in self.schema.all_interdental_gaps(region):
