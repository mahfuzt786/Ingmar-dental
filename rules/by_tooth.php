...
class StatusPwInPosteriorRegion {
    ...
    // const $RuleByToothInteface = new RuleByToothInteface()->run;

    function __construct($schema, $identified_subsidies) {
        $this->schema = $schema;
        $this->identified_subsidies = $identified_subsidies;
        $this->RuleByToothInteface = new RuleByToothInteface();
    }

    function execute_for($tooth) {
        $RegionGroup = new RegionGroup(
            new Region(18, 14, $this->schema),
            new Region(24, 28, $this->schema),
            new Region(38, 34, $this->schema),
            new Region(44, 48, $this->schema),
        );

        if (! in_array($tooth, $RegionGroup->get_teeth_with_condition("pw"))) {
            return False;
        }
...
class StatusPwInFrontRegion {
    ...
    function execute_for($tooth) {
        $RegionGroup = new RegionGroup(
            new Region(13, 23, $this->schema),
            new Region(33, 43, $this->schema)
        );
...
function evaluate_by_tooth($schema, $identified_subsidies) {
    ...
    // ];
    $rules = [
        new StatusPwInPosteriorRegion($schema, $identified_subsidies),
        new ToBeTreatedWithNoAbutmentTeethIncluded($schema, $identified_subsidies),
        new StatusPwInFrontRegion($schema, $identified_subsidies),
    ];

    $StatusPwInPosteriorRegion = new StatusPwInFrontRegion($schema, $identified_subsidies);

    foreach($rules as $rule_to_execute) {
        // print_r($rule_to_execute);
